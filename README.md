...
- **多账本**：可新建多个账本（分类/账户/流水各自独立），首页一键切换；支持「普通账本」「用车费用」「饮食账本」三种类型
- **用车费用账本**：记加油时录「油费类型 + 加油升数 + 行驶公里数」，自动算出百公里油耗与每公里费用；统计页展示「最近一次油耗 + 综合油耗 + 每公里费用」
- **饮食账本**：每顿饭「拍照 / 相册」选图，直连 DeepSeek 视觉模型识别碳蛋脂与热量并自动填充（可手动改）；首页看今日摄入，统计页看每日碳蛋脂趋势与日均摄入
...
数据（账本/流水/分类/账户/饮食记录）保存在本机浏览器 IndexedDB 中，不上传你的服务器。可在「我的 → 数据备份」中导出 JSON 文件；换设备时用「导入数据」恢复。

> 例外：饮食账本的「拍照识别」会把图片发送到 DeepSeek 视觉模型（OpenAI 兼容接口），需要你在「我的 → 饮食识别」里填写自己的 API Key；Key 仅存本机 localStorage、不进备份。
...
│   ├── utils/                     # 金额、日期、id、主题、油耗计算、饮食识别（DeepSeek）
