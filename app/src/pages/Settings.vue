    ...
    try {
      ...
      await db.transaction(
        ...
        async () => {
          ...
          if (Array.isArray(data.trips)) await db.trips.bulkAdd(data.trips);
...
// —— 同步到电脑（单向备份：手机 → 电脑端服务）——
const SYNC_SERVER_KEY = 'qtnotes-sync-server';
...
async function syncToServer() {
  ...
  const payload = { ledgers: ls, transactions: txs, categories: cats, accounts: accs, trips: trps, settings: sts };
