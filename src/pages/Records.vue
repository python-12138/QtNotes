...
function metaFor(t: Transaction): string {
  ...
  let meta = acc?.name ?? '';
  if (t.liters && t.km) {
    meta += (meta ? ' · ' : '') + `${t.liters}L · ${t.km}km · ${formatConsumption(fuelConsumption(t.liters, t.km))}`;
  }
...
<template>
  <div class="page records">
    ...
    <section v-for="[date, items] in groups" :key="date" class="day-group">
      ...
      <TransactionItem v-for="t in items" :key="t.id" :tx="t" :meta="metaFor(t)" />
    </section>
  </div>
</template>
